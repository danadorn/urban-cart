...
def run_phase4() -> None:
    ...
    reviews = pd.read_csv(OUT_DIR / "clean_reviews.csv")
    web_sessions = pd.read_csv(OUT_DIR / "clean_web_sessions.csv", parse_dates=["session_date"])
    rfm = pd.read_csv(OUT_DIR / "rfm_segments.csv")
    mc = pd.read_csv(OUT_DIR / "monte_carlo_stockout.csv")
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q1_revenue_by_segment.png", dpi=150)
    plt.close()
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q2_seasonality.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q3_margin_by_category.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q4_rating_vs_repeat.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q5_conversion_heatmap.png", dpi=150)
    plt.close()
    ...
    ax.plot(months_idx, np.column_stack([np.ones(len(months_idx)), months_idx]) @ beta,
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q6_forecast.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q7_stockout_risk.png", dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(FIG_DIR / "q8_returns_impact.png", dpi=150)
    plt.close()
    ...
    with open(OUT_DIR / "phase4_findings.txt", "w") as f:
        f.write("\n\n".join(findings))
